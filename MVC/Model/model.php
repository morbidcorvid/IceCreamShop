...
class Model 
{
	...
	public function saveOrder(Order $order){
		...
		try {
			...
			foreach ($order->items as $lineItem) {
				...
				//insert line_item
				$sql = sprintf("insert into line_item (order_id, item, qty, price) values  (%u, %u, %u, %u)", $orderID, $itemID, $lineItem->qty, $lineItem->price);
				$conn->exec($sql);
				$lineItemID = $this->connection->lastInsertId();
				//update line_item with discount if there
				if (!is_null($lineItem->discount)){
					$sql = sprintf("UPDATE line_item SET discount = %u WHERE id = %u", $lineItem->discount->id, $lineItemID);
					$conn->exec($sql);
				}
				...
				foreach ($lineItem->item->extras as $extra) {
					...
					$sql = sprintf("INSERT INTO item_extras (item, extra) values (%u, %u)",$itemID,$extraID);
					$conn->exec($sql);
				}
				...
				if (isset($lineItem->item->flavors)){
					foreach ($lineItem->item->flavors as $flavor) {
						...
						$extraID = key($filtered);
						// insert flavor
						$sql = sprintf("INSERT INTO scoop (item, flavor, qty) values (%u, %u, %u)",$itemID, $extraID, $flavor->qty);
						$conn->exec($sql);
					}
				}
			}
	...
	public function getOrders(){
		...
		//get item_extras
		$sql = "SELECT item, name, e_type FROM item_extras ie, extra e where ie.extra = e.id";
		$query = $this->connection->prepare($sql);
		$query->execute();
		//build itemExtras as array of arrays of Extra object with key as item id
		$itemExtras = array();
		foreach ($query as $row)
		{
			...
			array_push($itemExtras[$row['item']], new Extra($row['name'], $row['e_type']));
		}
		
		//get scoops
		$sql = "SELECT * FROM scoop";
		$query = $this->connection->prepare($sql);
		$query->execute();
		//build scoops as array of arrays of Flavor objects with key as item id
		$scoops = array();
		foreach ($query as $row)
		{
			if (! isset($scoops[$row['item']])) {
				$scoops[$row['item']] = array();
			}
			array_push($scoops[$row['item']], new Flavor($row['qty'], $row['flavor']));
		}
		
		//get items
		$sql = "SELECT i.id, name  FROM item i, item_type it WHERE i.item_type = it.id";
		$query = $this->connection->prepare($sql);
		$query->execute();
